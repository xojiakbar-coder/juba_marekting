...
export default function ImageOverlay({ data }) {
    ...
    return (
        <div>
            <div key={data?.id} className='relative w-full h-[30vh] md:h-[40vh] lg:h-[65vh] overflow-hidden rounded-3xl font-unbound'>
                <img
                    src={data?.photo}
                    alt="Background"
                ...
                <div className='absolute inset-0 flex ps-[8%] flex-col justify-center items-start bg-gradient-to-r from-black/80 via-black/30 to-black/10'>
                    <div className='hidden lg:flex flex-col space-y-2 w-1/2'>
                        ...
                        <h2 className='md:text-3xl lg:text-4xl xl:text-6xl text-white2 font-bold'>{data?.title}</h2>
                        <p className='text-white text-md'>{data?.short_description}</p>
            ...
            <div className='flex lg:hidden flex-col space-y-2 w-full mt-5'>
                ...
                <h2 className='text-3xl md:text-4xl lg:text-5xl text-white2 font-bold'>{data?.title}</h2>
                <p className='text-white text-md'>{data?.short_description}</p>
